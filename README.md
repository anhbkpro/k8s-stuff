...
| [`istioinaction/`](istioinaction/) | Practice log for *Istio in Action*: per-chapter folders (e.g. `ch5/`) with the actual manifests/commands run through, plus `services/` (sample apps) and `troubleshooting/` for gotchas hit along the way. |
| [`ansiblefordevops/`](ansiblefordevops/README.md) | Practice log for *Ansible for DevOps* (Jeff Geerling): per-chapter folders (`chNN/`) with the playbooks/inventories/roles built while following along, plus a README progress tracker. |
